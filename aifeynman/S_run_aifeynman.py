...
def run_AI_all(pathdir,filename,BF_try_time=60,BF_ops_file_type="14ops", polyfit_deg=4, NN_epochs=4000, PA=PA,
               torch_model_class: Optional[Callable[[Any], Module]]=None):
    try:
        os.mkdir("results/")
    except:
        pass
...
#############################################################################################################################
    ...
    if len(data[0])<3:
        ...
        print("NN loss: ", NN_eval(pathdir,filename, torch_model_class=torch_model_class)[0], "\n")
        model_feynman = NN_eval(pathdir,filename, torch_model_class=torch_model_class)[1]
    elif path.exists("results/NN_trained_models/models/" + filename + "_pretrained.h5"):
        print("Found pretrained NN \n")
        model_feynman = NN_train(pathdir,filename,NN_epochs/2,lrs=1e-3,N_red_lr=3,pretrained_path="results/NN_trained_models/models/" + filename + "_pretrained.h5", torch_model_class=torch_model_class)
        print("NN loss after training: ", NN_eval(pathdir,filename, torch_model_class=torch_model_class), "\n")
    else:
        print("Training a NN on the data... \n")
        model_feynman = NN_train(pathdir,filename,NN_epochs, torch_model_class=torch_model_class)
        print("NN loss: ", NN_eval(pathdir,filename, torch_model_class=torch_model_class), "\n")


    # Check which symmetry/separability is the best
    # Symmetries
    print("Checking for symmetries...")
    symmetry_minus_result = check_translational_symmetry_minus(pathdir,filename, torch_model_class=torch_model_class)
    symmetry_divide_result = check_translational_symmetry_divide(pathdir,filename, torch_model_class=torch_model_class)
    symmetry_multiply_result = check_translational_symmetry_multiply(pathdir,filename, torch_model_class=torch_model_class)
    symmetry_plus_result = check_translational_symmetry_plus(pathdir,filename, torch_model_class=torch_model_class)
    print("")

    print("Checking for separabilities...")
    # Separabilities
    separability_plus_result = check_separability_plus(pathdir,filename, torch_model_class=torch_model_class)
    separability_multiply_result = check_separability_multiply(pathdir,filename, torch_model_class=torch_model_class)
    ...
    if idx_min == 0:
        ...
        new_pathdir, new_filename = do_translational_symmetry_plus(pathdir,filename,symmetry_plus_result[1],symmetry_plus_result[2], torch_model_class=torch_model_class)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA1_, torch_model_class=torch_model_class)
        ...
        new_pathdir, new_filename = do_translational_symmetry_minus(pathdir,filename,symmetry_minus_result[1],symmetry_minus_result[2], torch_model_class=torch_model_class)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA1_, torch_model_class=torch_model_class)
        ...
        new_pathdir, new_filename = do_translational_symmetry_multiply(pathdir,filename,symmetry_multiply_result[1],symmetry_multiply_result[2], torch_model_class=torch_model_class)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA1_, torch_model_class=torch_model_class)
        ...
        new_pathdir, new_filename = do_translational_symmetry_divide(pathdir,filename,symmetry_divide_result[1],symmetry_divide_result[2], torch_model_class=torch_model_class)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir,new_filename,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA1_, torch_model_class=torch_model_class)
        ...
        new_pathdir1, new_filename1, new_pathdir2, new_filename2,  = do_separability_plus(pathdir,filename,separability_plus_result[1],separability_plus_result[2], torch_model_class=torch_model_class)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir1,new_filename1,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA1_, torch_model_class=torch_model_class)
        PA2_ = ParetoSet()
        PA2 = run_AI_all(new_pathdir2,new_filename2,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA2_, torch_model_class=torch_model_class)
        ...
        new_pathdir1, new_filename1, new_pathdir2, new_filename2,  = do_separability_multiply(pathdir,filename,separability_multiply_result[1],separability_multiply_result[2], torch_model_class=torch_model_class)
        PA1_ = ParetoSet()
        PA1 = run_AI_all(new_pathdir1,new_filename1,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA1_, torch_model_class=torch_model_class)
        PA2_ = ParetoSet()
        PA2 = run_AI_all(new_pathdir2,new_filename2,BF_try_time,BF_ops_file_type, polyfit_deg, NN_epochs, PA2_, torch_model_class=torch_model_class)
...
def run_aifeynman(pathdir,filename,BF_try_time,BF_ops_file_type, polyfit_deg=4, NN_epochs=4000, vars_name=[],test_percentage=20, torch_model_class: Optional[Callable[[Any], Module]]=None):
    ...
    PA = run_AI_all(pathdir, filename+"_train", BF_try_time, BF_ops_file_type, polyfit_deg, NN_epochs, PA=PA, torch_model_class=torch_model_class)
